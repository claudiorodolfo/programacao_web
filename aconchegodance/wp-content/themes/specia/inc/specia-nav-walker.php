 ...
 */
/** nav-menu-walker	*/
class specia_nav_walker extends Walker_Nav_Menu {
	...
	function start_el(&$output, $item, $depth = 0, $args = array(), $id = 0) {
		...
		$item_output .= ($args->has_children) ? '</a><span class="mobi_drop d-lg-none"><a href="#" class="fa fa-plus"></a></span>' : '</a>';
...
function specia_nav_menu_css_class($classes) {
...
add_filter('nav_menu_css_class', 'specia_nav_menu_css_class');

function specia_fallback_page_menu($args = array()) {
	...
	$list_args['title_li'] = '';
	$list_args['walker']   = new specia_walker_page_menu;
...
function specia_page_menu_args($args) {
...
add_filter('wp_page_menu_args', 'specia_page_menu_args');

class specia_walker_page_menu extends Walker_Page {
	function start_lvl(&$output, $depth = 0, $args = array()) {
		$indent = str_repeat("\t", $depth);
		$output .= "\n$indent<ul class='dropdown-menu'>\n";
	}
	function start_el(&$output, $page, $depth = 0, $args = array(), $current_page = 0) {
		...
		if ($args['has_children'] && $depth > 0) {
			...
			$caret = '<span class="mobi_drop d-lg-none"><a href="#" class="fa fa-plus"></a></span>';
		} else if ($args['has_children'] && $depth === 0) {
			$css_class[] = 'dropdown';
			$caret = '<span class="mobi_drop d-lg-none"><a href="#" class="fa fa-plus"></a></span>';
		}
