<?php
function gruj_header_setting( $wp_customize ) {
$selective_refresh = isset( $wp_customize->selective_refresh ) ? 'postMessage' : 'refresh';	
	...
	'hs_nav_contact_info2', 
		array(
			'label'	      => esc_html__( 'Hide / Show Section', 'gruj' ),
			'section'     => 'hdr_nav_contact_info2',
			'type'        => 'radio',
			'choices'        => 
			array(
...
add_action( 'customize_register', 'gruj_header_setting' );

// Header section selective refresh
function gruj_header_section_partials( $wp_customize ){
	...
	$wp_customize->selective_refresh->add_partial( 'hdr_nav_contact_ttl2', array(
		...
		'render_callback'  => 'gruj_hdr_nav_contact_ttl2_render_callback',	
	) );
	...
	$wp_customize->selective_refresh->add_partial( 'hdr_nav_contact_subttl2', array(
		...
		'render_callback'  => 'gruj_hdr_nav_contact_subttl2_render_callback',	
	) );
	}

add_action( 'customize_register', 'gruj_header_section_partials' );

// hdr_nav_contact_ttl2
function gruj_hdr_nav_contact_ttl2_render_callback() {
	return get_theme_mod( 'hdr_nav_contact_ttl2' );
}

// hdr_nav_contact_subttl2
function gruj_hdr_nav_contact_subttl2_render_callback() {
