<?php 
	$gruj_hs_service	= get_theme_mod('hide_show_service','on'); 
	$gruj_service_title	= get_theme_mod('service_title'); 
	$gruj_service_desc	= get_theme_mod('service_description');
	if($gruj_hs_service == 'on') :
?>
	<section id="unique-service" class="gruj-service service_flip_animate">		
		<div class="container">		
			<div class="row text-center padding-top-60 padding-bottom-30">
				<div class="col-sm-12">
				<?php if ($gruj_service_title) : ?>
					<h2 class="section-heading wow zoomIn"><?php echo wp_filter_post_kses($gruj_service_title); ?></h2>
				<?php endif; ?>
				
				<?php if ($gruj_service_desc) : ?>
					<p class="section-description"><?php echo esc_html($gruj_service_desc); ?></p>
				<?php endif; ?>
				</div>
			</div>
			<div class="row text-center padding-bottom-60">
				...
				<div class="col-md-4 col-sm-4 margin-bottom-30">
					...
					<?php if( !empty($image) ) { ?>
					<div class="service-box" style="background: url('<?php echo esc_url( $image ); ?>') no-repeat scroll 0 0 / cover rgba(0, 0, 0, 0);">
					<?php } else { ?>
						...
						<div class="back-part">
							<div class="back-content-part">
								...
								<div class="back-btn-part">
									<a class="back-btn" href="<?php echo esc_url( get_post_meta( get_the_ID(),'service_links', true) ); ?>"><?php _e( 'View More', 'gruj' ); ?></a>
								</div>
							</div>
						</div>
					</div>
